...
### 6.4 提交交付闭环（Push Gate）

- 每次完成 "提交"（包含根仓库与实现仓库）流程，默认必须立即执行双仓库 `push`，并按顺序执行：  
  1) 推送根仓库 `origin main`  
  2) 推送实现仓库 `origin main`  
- 任何情况下不得只执行 `commit` 而不执行对应的 `push`。
- 若任一 `push` 失败，立即停下后续动作，先修复失败原因并重试；待两仓库推送完成后才允许宣告任务交付完成。
